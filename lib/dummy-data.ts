...
export const dummyApplications: Application[] = [
  ...
  // 복수 필지 8건 신청 케이스 (복수 필지 개별 신청)
  {
    ...
    reportedShape: "삼각형",
    farmMachineDifficulty: false,
    reason: "동탄2 도시개발사업으로 인해 소유한 8개 필지가 모두 도��에 편입되어 잔여지가 불규칙한 형태로 남았습니다. 각 필지별로 건축 및 농업 활동이 불가능하여 일괄 매수를 신청합니다.",
    landDataList: [
      {
        currentUsage: "대" as const,
        landSubType: "residential-detached" as const,
  ...
  // 일단지 해당 케이스 2: 최혼합 - 4필지 혼합 (대지+농지+잡종지, 검토 중)
  {
    ...
    attachments: ["토지대장_777-1.pdf", "토지대장_777-2.pdf", "토지대장_777-3.pdf", "토지대장_777-4.pdf", "등기부등��.pdf", "건��물대장.pdf"],
    status: "검토중",
    adminStatus: "진행중",
  ...
  // 일단지 인정 케이스 2: 이산림 - 5필지 산지 (개별로는 면적 미달, 합산 시 조림지 분단으로 매수 인��)
  {
  ...
  // ===== 혼합 케이스: 일부 매수 + 일부 미해당 =====
  // ��혼합 - 4필지 모두 ��별 판정 (일단지 아님 - 지목/용도가 서로 다름)
  ...
  // 일단지 신청 -> 담당자 재분석으로 복수필지로 변경된 케이스
  {
    ...
    attachments: ["토지대장_200-1.pdf", "토지대장_200-2.pdf", "등기부등본.pdf"],
    status: "검토중",
    adminStatus: "진행중",
    ...
    aiResult: {
      ...
      unifiedLandAnalysis: {
        ...
        combinedArea: 330,
        explanation: "둔전리 200-1과 200-2는 동일 소유자, 연접, 동일 용도(대지)로 일단지 요건을 충족합니다. 합산 잔여면적 330㎡, 형상지수 4.5로 매수 기준에 해당합니다.",
      },
      landJudgments: [
        { landId: "land-unified-001", judgment: "매수", unifiedGroupId: "unified-dunjeon", reason: "일단지 구성 필지: 동일 소유자, 연접, 동일 용도(대지)" },
        { landId: "land-unified-002", judgment: "매수", unifiedGroupId: "unified-dunjeon", reason: "일단지 구성 필지: 동일 소유자, 연접, 동일 용도(대지)" },
      ],
    },
    // 담당자 AI 재분석 결과 - 일단지 부적합, 복수필지 개별 판정
    adminAiResult: {
      provisionalJudgment: "매수", // 전체 판정은 매수 (첫번째 필지 기준)
      confidence: 75,
      originalArea: 830,
      remainingArea: 330,
      remainingRatio: 39.8,
      ...
      shapeIndexChange: 1.6,
      isBlindLand: false,
      accessRoadLost: false, // 담당자가 접면도로 상실 없음으로 변경
      waterChannelLost: false,
      farmMachineDifficulty: false,
      judgmentRationale: {
        summary: "일단지 부적합: 현장 확인 결과 필지 간 2m 도로로 분리되어 연접 요건 미충족. 복수필지 개별 판정으로 변경",
        legalBasis: "「공익사업을 위한 토지 등의 취득 및 보상에 관한 법률」 제74조 및 동법 시행규칙 제34조",
        appliedCriteria: [
          "일단지 요건 미충족: 연접 ✗ (2m 도로로 분리)",
          "개별 판정으로 전환",
          "200-1: 잔여 180㎡ ≤ 200㎡(소규모), 형상지수 3.2(불량) → 매수",
          "200-2: 잔여 150㎡ ≤ 200㎡(소규모), 형상지수 2.5(양호) → 미해당",
        ],
        detailedExplanation: "현장 확인 결과, 둔전리 200-1과 200-2 사이에 2m 폭의 마을 도로가 존재하여 물리적으로 연접하지 않습니다. 따라서 일단지 요건을 충족하지 못하여 복수필지 개별 판정으로 변경합니다.\n\n[개별 판정 결과]\n• 200-1(대지): 잔여 180㎡로 소규모 토지 기준 충족, 형상지수 3.2(불량)로 건축 곤란 → 매수\n• 200-2(대지): 잔여 150㎡로 소규모 토지 기준 충족하나, 형상지수 2.5(양호)로 건축 가능 → 미해당",
        manualCheckItems: ["마을 도로 폭 재확인", "200-2 건축 가능 여부 현장 확인"],
      },
      // adminAiResult에는 unifiedLandAnalysis가 없음 (일단지 부적합이므로)
      landJudgments: [
        { landId: "land-unified-001", judgment: "매수", unifiedGroupId: null, reason: "개별 판정: 잔여 180㎡(소규모), 형상지수 3.2(불량), 건축 곤란" },
        { landId: "land-unified-002", judgment: "미해당", unifiedGroupId: null, reason: "개별 판정: 잔여 150㎡(소규모)이나 형상지수 2.5(양호), 건축 가능" },
      ],
    },
    adminName: "김철수",
