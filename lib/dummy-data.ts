...
export const dummyApplications: Application[] = [
  ...
  // 일단지 해당 케이스 1: 박일단 - 3필지 농지 (AI 분석 완료, 매수 판정)
  {
    ...
    aiResult: {
      ...
      farmMachineDifficulty: true,
      judgmentRationale: {
        summary: "일단지 해당 3필지 농지 - 모든 매수 기준 충족으로 「매수」 판정",
        legalBasis: "「공익사업을 위한 토지 등의 취득 및 보상에 관한 법률」 제74조 및 동법 시행규칙 제34조",
        appliedCriteria: [
          "일단지 판정: 동일 소유자, 연속 필지, 동일 용도(농지)",
          "합산 잔여면적: 570㎡ (개별 필지 기준 각각 충족)",
    ...
    finalJudgment: "매수",
    reviewerComment: "일단지 해당 3필지 농지로 확인됨. 관개수로 단절 및 형상 변경으로 농업 활동 불가. 매수 기준 충족으로 매수 결정.",
    finalReviewOpinion: "안성-천안 국도확장사업으로 편입된 3필지 농지입니다. 현장 확인 결과, 편입 전 일단지로 일괄 경작해 온 농지로 확인되었습니다. 도로 편입 후 관개수로가 단절되고, 각 필지가 삼각형, 역삼각형, 부정형으로 변경되어 농기계 진입 및 회전이 불가능한 상태입니다. 일단지 해당 조건을 모두 충족하여 매수가 적정합니다.",
    adminName: "홍길동",
  ...
  // 일단지 해당 케이스 2: 최혼합 - 4필지 혼합 (대지+농지+잡종지, 검토 중)
  {
    ...
    aiResult: {
      ...
      farmMachineDifficulty: true,
      judgmentRationale: {
        summary: "일단지 해당 4필지 혼합 토지 - 용도 상이하나 일체적 사용으로 일단지 판정, 「매수」 판정",
        legalBasis: "「공익사업을 위한 토지 등의 취득 및 보상에 관한 법률」 제74조 및 동법 시행규칙 제34조",
        appliedCriteria: [
          "일단지 판정: 동일 소유자, 연속 필지, 일체적 사용 (주택+텃밭+창고)",
          "합산 잔여면적: 450㎡ (개별 필지 기준 각각 충족)",
      ...
      unifiedParcelAnalysis: {
        ...
        conditions: {
          sameOwner: true,
          continuous: true,
          sameUsage: false, // 용도 상이하나 일체적 사용
        },
        combinedArea: 450,
        explanation: "동일 소유자(최혼합)의 연속된 4필지로, 대지(주택), 농지(텃밭), 잡종지(창고)를 일체적으로 사용해 왔으므로 일단지로 판정합니다. 용도는 상이하나 주거 목적의 일체적 토지 이용이 인정됩니다.",
      },
    },
    adminName: "홍길동",
  ...
  // 일단지 인정 케이스 1: 김인정 - 2필지 대지 (개별 면적 기준 미달이나 합산 시 맹지로 매수 인정)
  {
    ...
    aiResult: {
      landTypePath: "대지",
      criteriaChecks: [
        { criteriaName: "면적 기준 (개별)", criteriaDescription: "개별 필지 면적 기준 미달 (80㎡, 60㎡ > 90㎡ 기준)", isMet: false, autoDetected: true },
        { criteriaName: "일단지 합산 면적", criteriaDescription: "합산 잔여면적 140㎡로 건축 곤란", isMet: true, autoDetected: true },
        ...
        { criteriaName: "일단지 인정", criteriaDescription: "개별 기준 미달이나 일단지 합산 시 맹지로 매수 인정", isMet: true, autoDetected: true },
      ],
      provisionalJudgment: "매수",
      originalShapeIndex: 4.1,
      ...
      farmMachineDifficulty: false,
      judgmentRationale: {
        summary: "일단지 인정 2필지 대지 - 개별 면적 기준 미달이나 일단지 합산 시 맹지 판정으로 「매수」 인정",
        legalBasis: "「공익사업을 위한 토지 등의 취득 및 보상에 관한 법률」 제74조 및 동법 시행규칙 제34조, 중앙토지수용위원회 일단지 판정 기준",
        appliedCriteria: [
          "일단지 인정: 동일 소유자, 연속 필지, 동일 용도(주거용 대지)",
          "개별 면적: 각각 80㎡, 60㎡로 90㎡ 기준 미달",
          "합산 잔여면적: 140㎡ (건축 곤란)",
  ...
  // 일단지 인정 케이스 2: 이산림 - 5필지 산지 (개별로는 면적 미달, 합산 시 조림지 분단으로 매수 인정)
  {
    ...
    aiResult: {
      ...
      criteriaChecks: [
        ...
        { criteriaName: "일단지 인정", criteriaDescription: "개별 기준 미달이나 일단지 합산 시 조림지 분단으로 매수 인정", isMet: true, autoDetected: true },
      ],
      provisionalJudgment: "매수",
      ...
      farmMachineDifficulty: false,
      judgmentRationale: {
        summary: "일단지 인정 5필지 산지 - 개별 면적 기준 미달이나 일단지 합산 시 조림지 분단으로 「매수」 인정",
        legalBasis: "「공익사업을 위한 토지 등의 취득 및 보상에 관한 법률」 제74조 및 동법 시행규칙 제34조, 산지관리법 제18조",
        appliedCriteria: [
          "일단지 인정: 동일 소유자, 연속 필지, 동일 용도(조림지)",
          "개별 면적: 각각 500~1,500㎡로 990㎡ 기준 미달",
          "합산 잔여면적: 4,100㎡ (조림지 분단)",
          "토지 양분: 고속도로 관통으로 일체적 산림경영 불가",
          "※ 개별 기준으로는 매수 불가이나, 일단지 합산 시 조림지 분단으로 매수 인정",
        ],
        detailedExplanation: "일단지 인정 5필지 산지 (조림지)\n\n[필지 1] 산101: 3,000㎡ → 1,500㎡ (삼각형) - 면적 기준 미달\n[필지 2] 산102: 2,500㎡ → 700㎡ (역삼각형) - 면적 기준 충족\n[필지 3] 산103: 2,800㎡ → 800㎡ (부정형) - 면적 기준 충족\n[필지 4] 산104: 2,200㎡ → 600㎡ (삼각형) - 면적 기준 충족\n[필지 5] 산105: 1,800㎡ → 500㎡ (자루형) - 면적 기준 충족\n\n합산 편입전: 12,300㎡ / 합산 잔여: 4,100㎡\n\n※ 30년간 일괄 조림하여 관리해 온 산지로, 고속도로가 중앙을 관통하여 조림지가 양분되었습니다. 개별 필지 중 4필지는 면적 기준(990㎡)을 충족하나, 조림지 전체가 분단되어 일체적 산림경영이 불가능하므로 일단지 합산 시 매수가 인정됩니다.",
      ...
      unifiedParcelAnalysis: {
        ...
        combinedArea: 4100,
        explanation: "동일 소유자(이산림)의 연속된 5필지 산지로, 30년간 일괄 조림하여 관리해 온 임야입니다. 고속도로가 중앙을 관통하여 조림지가 양분되어 일체적 산림경영이 불가능합니다.",
      },
    },
    adminName: "박영희",
    statusUpdatedAt: "2026-04-29",
