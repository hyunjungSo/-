...
// AI 1차 판독 결과 (매수/매수불가/심의위원회 이관)
export type AIJudgmentResult = "매수" | "매수불가" | "심의위원회 이관";

// 최종 판정 결과 (매수/매수불가/심의위원회 이관)
export type FinalJudgmentResult = "매수" | "매수불가" | "심의위원회 이관";
...
export interface LandJudgment {
  landId: string; // 토지 ID
  judgment: "매수" | "매수불가" | "심의위원회 이관"; // 판정 결과
